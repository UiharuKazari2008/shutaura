...
(async () => {
    ...
    async function generateADSImage(imageBuffer, width, height, opts) {
        ...
        const blurredBackgroundBuffer = await sharp(imageBuffer)
            ...
            .blur(8)
            .modulate({
                brightness: (opts && opts.tint && opts.tint.d) ? 0.8 : 1,
                saturate: 2
            })
            .normalise((opts && opts.tint && opts.tint.d) ? { lower: 20, upper: 100 } : 1)
            .toBuffer();
        ...
        if (opts && opts.tint) {
            const increaseSaturation = (color) => Math.min(Math.floor(color * ((opts && opts.tint && opts.tint.d) ? 2.25 : 1.75)), 255);
        ...
        try {
            ...
            if (imageBuffer) {
                if (base64) {
                    ...
                    res.write(imageBuffer.toString('base64'))
                    res.end();
                } else {
