                    ...
                    if (full.length > 0 || previews.length > 0) {
                        ...
                        if (messages.rows.length > 0) {
                            ...
                            messages_verify.then(async () => {
                                if (deleteID.size > 0) {
                                    await db.query(`DELETE FROM kanmi_records_cdn WHERE (${Array.from(deleteID.keys()).map(e => 'heid = ' + (parseInt(e.toString()) * parseInt(systemglobal.CDN_ID.toString()))).join(' OR ')}) AND host = ?`, [systemglobal.CDN_ID]);
