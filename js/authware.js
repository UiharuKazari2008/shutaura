                ...
                if (data) {
                    try {
                        ...
                        if (member.roles.indexOf(data.role) === -1) {
                            if (data.approval === 1) {
                                try {
                                    const reqMsg = await discordClient.createMessage(data.approval_ch || staticChID.homeGuild.AlrmNotif, `🛎 User ${(member.nick) ? member.nick : member.user.username} from ${member.guild.name} is requesting permission "${(data.name) ? data.name : data.role}"`);
                                    if (reqMsg && reqMsg.id) {
    ...
    async function memberRoleGeneration(guild, member, dontUpdateAccount) {
        ...
        if (serverPermissions && serverPermissions.rows.length > 0) {
            ...
            for (const role of serverPermissions.rows.filter(e => e.name && member.roles.indexOf(e.role) !== -1 && ignoredPermissions.indexOf(e.name) === -1)) {
                ...
                await db.query(`INSERT INTO discord_users_permissons SET uid = ?, userid = ?, serverid = ?, color = ?, text = ?, role = ?, type = ?`, [md5(guild.id + member.user.id + type + role.role), member.user.id, guild.id, color, roleText, roleName, type]);
                if (roleName === 'user') {
                    await db.query(`INSERT INTO discord_users_permissons SET uid = ?, userid = ?, serverid = ?, color = ?, text = ?, role = ?, type = ?`, [md5(guild.id + member.user.id + type + role.role + 'userGeneric'), member.user.id, guild.id, color, roleText, `${roleName}-${guild.id}`, type]);
                }
            }
        }
    ...
    async function sequenziaUserCacheGenerator(thisUser) {
        ...
        for (let userId of users) {
            ...
            let userAccount = {
                ...
                server_list: [],
                cache: {
                    channels_view: `ZENLESS__auth_${userId}`,
            ...
            await new Promise(async (resolve) => {
                ...
                if (tempLastEpisode.rows.length > 0) {
                    ...
                    if (nextEpisodeView.rows[0]) {
                        userAccount.kongou_next_episode = {
                            ...nextEpisodeView.rows[0],
                            background: (nextEpisodeView.rows[0].background) ? `${systemglobal.CDN_RemoteURL}kongou/backdrop/${nextEpisodeView.rows[0].dat_1_hint}` : null,
                            poster: (nextEpisodeView.rows[0].poster) ? `${systemglobal.CDN_RemoteURL}kongou/poster/${nextEpisodeView.rows[0].dat_0_hint}` : null
                        };
                    } else {
                        userAccount.kongou_next_episode = {};
                    }
                ...
                const userArtists = await db.query(`SELECT rec.*, cdn.host AS cdn_host, cdn.path_hint, cdn.mfull_hint, cdn.full_hint, cdn.preview_hint, cdn.ext_0_hint FROM (SELECT * FROM (SELECT kanmi_records.eid, kanmi_records.attachment_hash, kanmi_records.attachment_name, kanmi_records.cache_proxy, kanmi_records.sizeH, kanmi_records.sizeW, kanmi_records.sizeR, kanmi_records.colorR, kanmi_records.colorG, kanmi_records.colorB, sequenzia_index_artists.id AS artist_id, sequenzia_index_artists.artist, sequenzia_index_artists.name AS artist_full_name, sequenzia_index_artists.url AS artist_url, sequenzia_index_artists.search AS artist_search, sequenzia_index_artists.count AS artist_count, sequenzia_index_artists.last AS artist_last, sequenzia_index_artists.source AS artist_source, sequenzia_index_artists.confidence AS artist_confidence, sequenzia_index_artists.rating AS artist_rating, ZENLESS__auth_${userId}.* FROM kanmi_records,ZENLESS__auth_${userId}, sequenzia_index_artists  WHERE (sequenzia_index_artists.channelid = ZENLESS__auth_${userId}.channelid AND kanmi_records.eid = sequenzia_index_artists.last AND kanmi_records.channel = ZENLESS__auth_${userId}.channelid)) x INNER JOIN (SELECT id AS fav_id, date AS fav_date FROM sequenzia_artists_favorites WHERE userid = "${userId}") y ON x.artist_id = y.fav_id ORDER BY x.artist_last DESC) rec LEFT OUTER JOIN (${selectCDN}) cdn ON (rec.eid = cdn.eid)`)
                const channelFolders = await db.query(`SELECT f.fid, f.name, f.icon, f.c_lock, c.cid, c.channelid FROM (SELECT cid, channelid FROM kanmi_channels) c JOIN (SELECT * FROM kanmi_virtual_folders) f ON (c.cid = f.cid)`);

                const libraryLists = await db.query(`SELECT g.* FROM (SELECT * FROM kongou_media_groups) g INNER JOIN (SELECT media_group FROM ZENLESS__auth_${userId}) a ON (g.media_group = a.media_group) GROUP BY g.media_group`)
            ...
            if (req.body && req.body.key && req.body.id && req.body.users) {
                if (systemglobal.Authorized_Exchange && systemglobal.Authorized_Exchange[req.body.id] && systemglobal.Authorized_Exchange[req.body.id]['key'] === req.body.key) {
                    ...
                    res.status(200).json({
                        ...
                        config: {
                            base_url: systemglobal.This_Exchange.base_url || systemglobal.base_url,
                            access_url: systemglobal.This_Exchange.access_url || systemglobal.This_Exchange.base_url || systemglobal.base_url
                        },
                        version: '1'
                    })
                } else {
                        ...
                        request.post({
                            ...
                            if (err || res && res.statusCode && res.statusCode !== 200) {
                                ...
                                try {
                                    ...
                                    Logger.printLine("Exchange", `Failed to parse response from exchange "${id}" - Status: ${(res && res.statusCode) ? res.statusCode : 'Unknown'}`, "err", (e) ? e : undefined)
                                    console.error(e)
                                    json(null)
                                }
                            }
