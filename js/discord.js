                                                        ...
                                                        if (data && data.attachments.length > 0) {
                                                            ...
                                                            if (attachmentUrl) {
                                                                ...
                                                                try {
                                                                    ...
                                                                    const date = new Date(parseInt(exSearch.get('ex') || '', 16) * 1000);
                                                                    const exTime = moment(date).local().format('YYYY-MM-DD HH:mm:ss');
                ...
                if (MessageContents.messageRefrance && MessageContents.messageRefrance.action && MessageContents.messageRefrance.action === 'jfsMove' ) {
                    ...
                    if (url.length === 2) {
                        try {
                            ...
                            const date = new Date(parseInt(exSearch.get('ex') || '', 16) * 1000);
                            exTime = moment(date).local().format('YYYY-MM-DD HH:mm:ss');
                        } catch (err) {
                            Logger.printLine("Discord", `Failed to get auth expire time value for database row!`, "debug", err);
                        }
                    }
                ...
                if (err || cacheresponse.length === 0) {
                    ...
                    part_download.then(async () => {
                        if (cacheresponse[0].paritycount && filelist.length !== cacheresponse[0].paritycount) {
                            ...
                            res.status(200).json({
                                error: false,
                                parts: filelist.map(e => `${(req.query && req.query.proxy) ? decodeURIComponent(req.query.proxy) : ''}/attachments${e.split('/attachments').pop()}`).sort((x, y) => (x.split('.').pop().split('?')[0] < y.split('.').pop().split('?')[0]) ? -1 : (y.split('.').pop().split('?')[0] > x.split('.').pop().split('?')[0]) ? 1 : 0),
