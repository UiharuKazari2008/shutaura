...
const stage1DatabaseUpdate = [
    {
        table: 'discord_multipart_files',
        add: [
            'parity_index MEDIUMINT not null after fileid',
            'parity_hash TINYTEXT not null after url',
            'parity_name TINYTEXT not null after parity_hash'
        ],
        drop: [],
        modify: [
            'messageid tinytext not null',
            'channelid tinytext null',
            'serverid tinytext null',
            'fileid tinytext not null',
            'url tinytext null',
            'hash tinytext null'
        ]
    },
...
module.exports = function (Logger, db) {
    ...
    module.patchDatabase = async function patchDatabase() {
        try {
            const sqlStructure = await db.query(`SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME=?`, [systemglobal.SQLDatabase, 'discord_multipart_files'])
            if (sqlStructure.error) {
                Logger.printLine("SQL", "Failed to check database", "error")
                console.log(sqlStructure.error)
                return false
            ...
            if (columns.indexOf('url') === -1) {
                Logger.printLine("Update", "Skipping updating parity database to v2", "info");
            } else {
                Logger.printLine("SQL", "Getting Parity Files to hashed and indexed...", "debug")
                const cachedFiles = (await db.query(`SELECT * FROM discord_multipart_files WHERE parity_index is NULL OR parity_hash IS NULL`)).rows
                if (cachedFiles.length > 0) {
                    Logger.printLine("Update", `Updating ${cachedFiles.length} parts...`, "debug")
                    for (const i in cachedFiles) {
                        try {
                            const urlParts = cachedFiles[i].url.split('attachments').pop().split('/')
                            const partIndex = urlParts[2].split('.').pop()
                            // https://cdn.discordapp.com/attachments/ [0]channel / [1]hash / [2]name
                            let updateRow = {
                                url: null,
                                parity_hash: urlParts[1],
                                parity_name: urlParts[2],
                            }
                            if (urlParts[0] !== cachedFiles[i].channelid) {
                                updateRow.channelid = urlParts[0];
                                Logger.printLine("Update", "Channel ID mismatch, will overwrite but may be a issue when attempting to delete the file later - " + urlParts.join("/"), "info");
                            }
                            if (partIndex.startsWith("sf-part")) {

                            } else if (partIndex.startsWith("PSF-")) {
